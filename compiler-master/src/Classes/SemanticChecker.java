...
public class SemanticChecker {
    ...
    private void checkComponents() {
        ...
        for (var row : symbolTable.getRows()) {
            System.out.println("فحص صف من نوع: " + row.type);

            if (row.type.equals("Component")) {
                foundComponents = true;
                System.out.println("تم العثور على كومبوننت: " + row.value);

                String[] parts = row.value.split("\\s+");
                if (parts.length < 2) {
                    errors.add("خطأ: تعريف كومبوننت غير صحيح: '" + row.value + "'");
                    errorTable.addRow(row);
                    hasError = true;
                    continue;
                }
                ...
                // التحقق من خصائص الكومبوننت
                if (!validateComponentProperties(row.value)) {
                    errors.add("خطأ: خصائص كومبوننت غير صحيحة: '" + row.value + "'");
                    errorTable.addRow(row);
                    hasError = true;
                }
                ...
                if (!isValidComponentName(componentName)) {
                    errors.add("خطأ: اسم كومبوننت غير صالح: '" + componentName + "'");
                    errorTable.addRow(row);
                    hasError = true;
                }

                // التحقق من وجود خصائص إلزامية
                if (!hasRequiredProperties(row.value)) {
                    errors.add("خطأ: خصائص إلزامية مفقودة في الكومبوننت: '" + componentName + "'");
                    errorTable.addRow(row);
                    hasError = true;
                }
            }
        ...
        if (!foundComponents) {
            System.out.println("لم يتم العثور على أي كومبوننت للفحص");
        }
